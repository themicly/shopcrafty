...
@section('content')
    ...
    {{-- Studio product page — classic fashion PDP. Left: squared gallery with a
         thumbnail rail. Right: caps breadcrumb, uppercase serif title, bold ink
         price, squared black-fill option chips and a solid black buy row. --}}
    <div class="st-container" style="padding-block: clamp(2.5rem, 6vw, 4rem); background: var(--st-bg)">

        {{-- Breadcrumb: quiet micro-caps --}}
        <nav class="stt-studio-crumb st-reveal mb-8 flex flex-wrap items-center gap-2" aria-label="Breadcrumb">
            <a href="{{ url('/shop') }}">{{ __('storefront.shop') }}</a>
            <span aria-hidden="true">/</span>
            <span style="color: var(--st-ink)">{{ $product->name }}</span>
        </nav>

        <div class="stt-studio-pdp-grid grid gap-y-10 lg:grid-cols-2" style="column-gap: 2.5rem">
            {{-- Gallery --}}
            <div class="st-reveal">
                    ...
                    <div class="aspect-[4/5]">
                        @if ($first)
                            ...
                            <button type="button" @click="lightbox = true" aria-label="{{ __('storefront.zoom_image') }}"
                                ...
                                style="background: color-mix(in srgb, #ffffff 88%, transparent); color: var(--st-ink); border: 1px solid var(--st-line); border-radius: var(--st-radius)">
                        ...
                        @else
                            <div class="stt-studio-crumb grid h-full w-full place-items-center">{{ __('storefront.no_image') }}</div>
                        @endif
                    </div>
                </figure>
                ...
                @if (count($images) > 1)
                    ...
                    <div class="mt-4 flex gap-3 overflow-x-auto pb-1">
                        @foreach ($images as $img)
                            <button type="button" @click="active = @js($img)" class="h-20 w-16 shrink-0 overflow-hidden border transition hover:opacity-100"
            ...
            {{-- Details --}}
            <div class="st-reveal stt-studio-pdp-sticky">
                <p class="stt-studio-eyebrow mb-4">{{ $product->category->name ?? 'The collection' }}</p>

                <h1 class="stt-studio-display" style="font-size: clamp(1.5rem, 3vw, 2.2rem)">{{ $product->name }}</h1>
                ...
                {{-- Price: bold ink, struck compare beside --}}
                <div class="mt-5">
                    @if ($optionGroups)
                        <span class="stt-studio-price" style="font-size: clamp(1.4rem, 2.8vw, 1.8rem)" x-text="current ? current.price : @js(format_money($product->price))"></span>
                    @else
                        <span class="flex items-baseline gap-3">
                            <span class="stt-studio-price" style="font-size: clamp(1.4rem, 2.8vw, 1.8rem)">{{ format_money($product->price) }}</span>
                            @if ($product->compare_at_price && $product->compare_at_price > $product->price)
                                <span class="stt-studio-price-was" style="font-size: 1.05rem">{{ format_money($product->compare_at_price) }}</span>
                                <span class="stt-studio-badge" style="background: var(--st-accent)">-{{ (int) round((1 - $product->price / $product->compare_at_price) * 100) }}%</span>
                            @endif
                        </span>
                    @endif
                </div>
                ...
                @if ($optionGroups)
                    ...
                    <div class="mt-3 text-sm font-semibold" x-show="current && block" x-cloak>
                        <template x-if="current && current.stock <= 0"><span style="color: #b3261e">{{ __('storefront.sold_out') }}</span></template>
                        <template x-if="current && current.stock > 0 && current.stock <= 10"><span style="color: #b3261e">Only <span x-text="current.stock"></span> left — order soon</span></template>
                        <template x-if="current && current.stock > 10"><span style="color: var(--st-accent)">{{ __('storefront.in_stock') }}</span></template>
                    </div>
                @elseif ($product->track_inventory)
                    <div class="mt-3 text-sm font-semibold">
                        @if ($product->stock_qty <= 0)
                            <span style="color: #b3261e">{{ __('storefront.sold_out') }}</span>
                        @elseif ($product->stock_qty <= 10)
                            <span style="color: #b3261e">{{ __('storefront.only_left_order_soon', ['count' => $product->stock_qty]) }}</span>
                        @else
                            <span style="color: var(--st-accent)">{{ __('storefront.in_stock') }}</span>
                        @endif
                    </div>
                @endif

                {{-- Per-attribute selectors — squared chips, black-filled when active;
                     colour swatches ring in ink. --}}
                @if ($optionGroups)
                    <div class="mt-8 space-y-6">
                        @foreach ($optionGroups as $group)
                            <div>
                                <p class="stt-studio-crumb mb-3" style="color: var(--st-ink)">
                                    {{ $group['name'] }}
                                ...
                                <div class="flex flex-wrap gap-2.5">
                                    @foreach ($group['values'] as $val)
                                        ...
                                        @else
                                            <button type="button" @click="selected[@js($group['name'])] = @js($val['value'])"
                                                :disabled="! optionAvailable(@js($group['name']), @js($val['value']))"
                                                class="stt-studio-chip disabled:cursor-not-allowed"
                                                :class="[selected[@js($group['name'])] === @js($val['value']) && 'stt-studio-chip--active', ! optionAvailable(@js($group['name']), @js($val['value'])) && 'line-through opacity-40']"
                        ...
                        @endforeach
                        <p x-show="needsSelection" x-cloak class="text-sm font-semibold" style="color: #b3261e">{{ __('storefront.select_options_first') }}</p>
                    </div>
                @endif
                ...
                <button type="button"
                    ...
                    class="stt-studio-btn mt-3 flex w-full"
                    style="background: var(--st-accent); border-color: var(--st-accent); color: #fff">
                    {{ __('storefront.buy_it_now') }}
                </button>

                @if ($whatsapp)
                    {{-- WhatsApp / ask / share — quiet outlined row --}}
                    @if (settings('general.whatsapp_buy_enabled', true))
                    <a :href="waBuy()" target="_blank" rel="noopener" class="stt-studio-btn-outline mt-5 flex w-full" style="color: var(--st-ink); min-height: 2.5rem; padding-top: 0.5rem; padding-bottom: 0.5rem; font-size: 12px">
                    ...
                    <div class="mt-3 flex gap-3">
                        @if (settings('general.whatsapp_inquiry', true))
                            <a href="https://wa.me/{{ $waNum }}?text={{ urlencode('Hi, I have a question about ' . $product->name . ' — ' . $waUrl) }}"
                                class="stt-studio-btn-outline flex-1" style="color: var(--st-ink)">{{ __('storefront.ask_a_question') }}</a>
                        @endif
                        @if (settings('general.whatsapp_share', true))
                            <button type="button" @click="stShare(@js($product->name), @js($waUrl), @js(__('storefront.link_copied')))"
                                class="stt-studio-btn-outline flex-1" style="color: var(--st-ink)">{{ __('storefront.share') }}</button>
                        @endif
                    </div>
                @endif

                {{-- Trust row — three quiet caps notes on the sage band --}}
                <div class="stt-studio-band mt-8 grid grid-cols-3 gap-4 px-4 py-4 text-center" style="border-radius: var(--st-radius)">
                    @foreach ([__('storefront.secure_checkout'), __('storefront.fast_delivery'), __('storefront.easy_returns')] as $t)
                        <div class="stt-studio-crumb" style="color: var(--st-ink)">{{ $t }}</div>
                    @endforeach
                </div>

                {{-- Description / shipping — squared segmented tabs, active fills black --}}
                @php
                ...
                <div class="mt-8" x-data="{ tab: 'desc' }">
                    <div class="inline-flex gap-1 p-1" style="border: 1px solid var(--st-line); border-radius: var(--st-radius)">
                        <button type="button" @click="tab = 'desc'" class="stt-studio-chip" style="border: 0"
                            :class="tab === 'desc' && 'stt-studio-chip--active'">{{ __('storefront.description') }}</button>
                        <button type="button" @click="tab = 'ship'" class="stt-studio-chip" style="border: 0"
                            :class="tab === 'ship' && 'stt-studio-chip--active'">{{ __('storefront.shipping_returns') }}</button>
                    </div>
    ...
    {{-- Frequently bought together — grey band, underlined head --}}
        ...
        <section class="stt-studio-section stt-studio-band-grey">
            <div class="st-container">
                <div class="stt-studio-head st-reveal">
                    <h2 class="stt-studio-title stt-studio-title--rule">{{ __('storefront.frequently_bought_together') }}</h2>
                </div>
                <div class="stt-studio-grid">
                    @foreach ($fbt as $p)
    ...
    @if (isset($recentlyViewed) && $recentlyViewed->isNotEmpty())
        <section class="stt-studio-section" style="background: var(--st-bg)">
            <div class="st-container">
                <div class="stt-studio-head st-reveal">
                    <h2 class="stt-studio-title stt-studio-title--rule">{{ __('storefront.recently_viewed') }}</h2>
                </div>
                <div class="stt-studio-grid">
